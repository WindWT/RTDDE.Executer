...
WHERE uo.id={0}";
            ...
            taskSkills.ContinueWith(t =>
            {
                ...
                UnitInfo_EXP_Next.Text = Math.Abs(thislevel - maxlevel) < 0.1 ? "MAX" : (expNextLevel - 1).ToString("0");
                ...
                if (taskAccessory.Result == null) {
                    ...
                    accessory_specified_attr.Text = Utility.ParseAttributeToString(acce.specified_attr);
                    accessory_specified_style.Text = Utility.ParseStyletype(acce.specified_style);
                }
                //Advanced
